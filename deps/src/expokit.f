...
*
      ...
      do 200 j = 1,m
         ...
         call matvec( wsp(j1v-n), wsp(j1v), arg )
         if ( j.gt.1 )
     .     call ZAXPY(n,-REAL(wsp(ih+(j-1)*mh+j-2)),wsp(j1v-2*n),1,wsp(j1v),1)
...
*---     if `happy breakdown' go straightforward at the end ...
         ...
         j1v = j1v + n
 200  continue
      wsp(ih+mbrkdwn*mh+mbrkdwn-1) = CMPLX(REAL(wsp(ih+mbrkdwn*mh+mbrkdwn-1)), 0 )
      nmult = nmult + 1
      call matvec( wsp(j1v-n), wsp(j1v), arg )
      avnorm = DZNRM2( n, wsp(j1v),1 )
*
